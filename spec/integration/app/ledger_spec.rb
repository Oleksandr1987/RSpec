  ...
  RSpec.describe Ledger, :aggregate_failures, :db do
    ...
    describe '#expenses_on' do
      it 'returns all expenses for the provided date' do
        result_1 = ledger.record(expense.merge('date' => '2022-08-11'))
        result_2 = ledger.record(expense.merge('date' => '2022-08-11'))
        result_3 = ledger.record(expense.merge('date' => '2022-08-12'))

        expect(ledger.expenses_on('2022-08-11')).to contain_exactly(
      ...
      it 'returns a blank array when there are no matching expenses' do
        expect(ledger.expenses_on('2022-08-11')).to eq([])
      end
    end
  end
end
